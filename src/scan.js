...
export function main(ns) {
  const factionServers = ["CSEC", "avmnite-02h", "I.I.I.I", "run4theh111z", "w0r1d_d43m0n", "fulcrumassets"],
    ...
    createServerEntry = (serverName) => {
      let server = serverInfo(serverName),
        ...
        shouldBackdoor =
          !server?.backdoorInstalled && canHack && serverName != "home" && rooted && !server.purchasedByPlayer,
    ...
    ordering = (serverA, serverB) => {
      ...
      orderNumber =
        orderNumber != 0 ? orderNumber : serverInfo(serverB).purchasedByPlayer - serverInfo(serverA).purchasedByPlayer
  ...
  doc
    .querySelectorAll(".serverscan.new .server")
    .forEach((serverEntry) =>
      serverEntry.addEventListener("click", setNavCommand.bind(null, routes[serverEntry.childNodes[0].nodeValue])),
    )
  doc
    .querySelectorAll(".serverscan.new .backdoor")
    .forEach((backdoorButton) =>
      backdoorButton.addEventListener(
        "click",
        setNavCommand.bind(null, routes[backdoorButton.parentNode.childNodes[0].childNodes[0].nodeValue] + ";backdoor"),
      ),
    )
